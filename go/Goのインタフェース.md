...
- [[インタフェース型]]が特定の具象型を持っているか
- その具象型が別のインタフェースを実装しているか

を調べるのに、Goには2つの方法がある。

## 型アサーション

**[[型アサーション]]** (type assertion) を使うと、インタフェースの値のもとになっている具体的な値を利用できる。

構文 [^2024-12-30-120401]

[^2024-12-30-120401]: https://go-tour-jp.appspot.com/methods/15

```go
// インタフェースの値 x が、具体的な型 T を保持し、基となる T の値を変数 t に代入することを主張
t := x.(T)
```
...
`x` が `T` を保持していない場合 panic を引き起こす。panic を回避するには[[カンマokイディオム]]を使う。

```go
t, ok := x.(T)
```

`x` が `T` を保持していれば `t` は基になる値となり、`ok` は `true` になる。
`x` が `T` を保持していなければ `t` は型 `T` のゼロ値となり、`ok` は `false` になる。
