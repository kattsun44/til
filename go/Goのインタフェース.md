...
[^2024-12-24-084804]: 特によく使われるのが [[interface{}]]。これは「0個のメソッドが定義された型」という意味になり、任意の型がこの条件を満たすため任意の型の値を記憶できる。詳しくは[空インタフェース](#空インタフェース)を参照。
...
インタフェースが [[nil]] であるとみなされるためには、型と値の両方が `nil` でなければならない。インタフェースは「ベースとなる型 ([基底型](Goの型#基底型)) へのポインタ」と「ベースとなる値へのポインタ」の組で実装されており、型が `nil` でなければインタフェースは `nil` にはならない[^2024-12-27-083614]。

[^2024-12-27-083614]: 変数は必ず型を持つため、値のポインタが `nil` でなければ型のポインタも必然的に非 `nil` となる。

インタフェースが `nil` でなければ、具象型の[インスタンスが nil であっても](Goのメソッド.md#nilへの対応)インタフェースに関するメソッドを起動できる。インタフェースが `nil` であれば、それに関するどのメソッドを起動してもパニックになる。

非 `nil` 型を持つインタフェースのインスタンスは `nil` と等しくないため、そのインタフェースに結びつけられた値が `nil` であるかどうかは ([[リフレクション]]を使わないと) 判断できない。

## 空インタフェース

[[interface{}]] は「[[空インタフェース]] (empty interface) 型」と呼ばれ、「0個以上のメソッドを実装した型」ならどのような値でも保持できることを表している。Go の任意の型 (any type) はこの条件を満たしている。[[Go 1.18]] からは `interface{}` の代わりに [[any]] と書けるようになった。

### 空インタフェースの用途

用途1 … 外部ソース (e.g. JSONファイル) から読み込まれた、形式不明なデータの記憶場所
...
json.Unmarchal(contets, &data)
fmt.Println(data)
```

用途2 … ユーザーが生成したデータ構造の中に値を保存 (スライス、配列、マップに収まらないが一つの型だけでしか使えないようにしたくない場合)
